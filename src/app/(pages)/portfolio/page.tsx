...
export default function Portfolio() {
  ...
  className="pt-40 pb-20 px-8 max-w-screen-2xl mx-auto">
    <div className="max-w-4xl">
      <span className="text-primary font-label tracking-[0.2em] uppercase text-xs mb-4 block">Creative Portfolio</span>
      <h1 className="text-5xl md:text-8xl font-black tracking-tighter mb-8 leading-15 md:leading-25">
        Digital <br /><span className="text-transparent bg-clip-text bg-linear-to-r from-primary to-secondary">Architectures.</span>
      </h1>
      <p className="text-on-surface/60 text-lg md:text-xl max-w-2xl leading-relaxed">
        ...
        realities. Explore our curated selection of high-fidelity interfaces
  ...
  <section className="px-8 mb-16 max-w-screen-2xl mx-auto">
    <div className="flex flex-wrap items-center gap-3">
      <button onClick={()=>setNav('all')} className={`px-6 py-2 rounded-full cursor-pointer ${nav =='all' ? 'bg-primary-container' : 'bg-surface-container-high'} text-primary font-semibold text-sm transition-all duration-300`}>
        All Work
      </button>
      <button onClick={()=>setNav('web')} className={`px-6 py-2 rounded-full cursor-pointer ${nav =='web' ? 'bg-primary-container' : 'bg-surface-container-high'} text-primary font-semibold text-sm transition-all duration-300`}>
       Web
      </button>
      <button onClick={()=>setNav('ads')} className={`px-6 py-2 rounded-full cursor-pointer ${nav =='ads' ? 'bg-primary-container' : 'bg-surface-container-high'} text-primary font-semibold text-sm transition-all duration-300`}>
       Ads
      </button>
      <button onClick={()=>setNav('branding')} className={`px-6 py-2 rounded-full cursor-pointer ${nav =='branding' ? 'bg-primary-container' : 'bg-surface-container-high'} text-primary font-semibold text-sm transition-all duration-300`}>
       Branding
      </button>
      <button onClick={()=>setNav('motion')} className={`px-6 py-2 rounded-full cursor-pointer ${nav =='motion' ? 'bg-primary-container' : 'bg-surface-container-high'} text-primary font-semibold text-sm transition-all duration-300`}>
       Motion
      </button>
  ...
  className="mt-40 px-8 text-center max-w-screen-2xl mx-auto">
    <div className="py-24 rounded-2xl bg-surface-container-low relative overflow-hidden">
      ...
      <h2 className="text-4xl md:text-6xl font-black tracking-tighter leading-10 md:leading-15 mb-8 max-w-3xl mx-auto">
        Ready to build the <span className="text-primary">extraordinary?</span>
      </h2>
    

        <Link href={'/contact'} className="mainBtn md:text-[20px] hover:-transale-1.25 hover:scale-110 transition-all duration-500 cursor-pointer px-6 py-2.5 md:px-10 md:py-4     rounded text-white font-bold  active:scale-90  hover:shadow-[0_0_30px_rgba(186,171,255,0.2)]">
